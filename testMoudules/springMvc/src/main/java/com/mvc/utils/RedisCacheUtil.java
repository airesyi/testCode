...
public class RedisCacheUtil {
    ...
    public boolean delStringValue(String key) {
    ...
    public boolean expireStringValue(String key, int seconds) {
        Long result = null;
        ShardedJedis client = shardedJedisPool.getResource();
        boolean exist = false;
        try {
            if (client.expire(key, seconds) > 0) {
                exist = true;
            }
        } catch (Exception e) {
            logger.error("{}", e);
    ...
    public void batchDel(String key) {
        ...
        try {
            ...
            for (Jedis jedis : jediss) {
                delSet.addAll(jedis.keys(key + "*"));
            }

//            delSet = client.getShard("a").keys("*");
            Iterator<String> it = delSet.iterator();
            while (it.hasNext()) {
